import { Contract, RpcProvider, Account, CairoCustomEnum } from "starknet";
...
export class StateTransitionService {
  ...
  constructor(
    ...
    this.vaultContract = new Contract(VaultAbi, vaultAddress, this.account);
  ...
  private async handleOpenState(
    ...
    try {
      ...
      if (reservePrice === 0n) {
        ...
        const requestData =
          await this.vaultContract.get_request_to_start_first_round();
        ...
        await this.sendFossilRequest(
          this.formatRawToFossilRequest(requestData),
        );
      ...
      if (latestBlockTimestamp < auctionStartTime) {
        this.logger.info(
          `Waiting for auction start time. Time left: ${this.formatTimeLeft(latestBlockTimestamp, auctionStartTime)}`,
        );
        return;
      }
  ...
  private async handleAuctioningState(
    roundContract: Contract,
    latestBlockTimestamp: number,
  ): Promise<void> {
    try {
      ...
      if (latestBlockTimestamp < auctionEndTime) {
        this.logger.info(
          `Waiting for auction end time. Time left: ${this.formatTimeLeft(latestBlockTimestamp, auctionEndTime)}`,
        );
        return;
      }
  ...
  private async fossilHasAllBlocks(requestTimestamp: number): Promise<boolean> {
    ...
    const latestFossilBlockTimestamp =
      latestFossilBlockResponse.data.block_timestamp;
  ...
  private async sendFossilRequest(requestData: FossilRequest): Promise<void> {
    ...
    const clientAddressRaw =
      await this.vaultContract.get_l1_data_processor_address();
    ...
    // Get alpha and k from vault contract
    const alpha = Number(await this.vaultContract.get_alpha());
    const k = Number(await this.vaultContract.get_strike_level());

    // Calculate windows for each metric
    const twapWindow = roundDuration;
    const capLevelWindow = roundDuration * 5;
    const reservePriceWindow = 3600 * 24 * 150; // 150 days
    ...
    const fossilRequest = {
      ...
      params: {
        ...
        cap_level: [requestTimestamp - capLevelWindow, requestTimestamp],
        reserve_price: [
          requestTimestamp - reservePriceWindow,
          requestTimestamp,
        ],
        alpha,
        k,
    ...
    console.log("Fossil request", fossilRequest);
  ...
  private async handleRunningState(
    roundContract: Contract,
    latestBlockTimestamp: number,
  ): Promise<void> {
    try {
      ...
      if (latestBlockTimestamp < settlementTime) {
        this.logger.info(
          `Waiting for settlement time. Time left: ${this.formatTimeLeft(latestBlockTimestamp, settlementTime)}`,
        );
        return;
      }
